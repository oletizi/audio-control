...
def find_plugin_by_name(plugin_name: str) -> Optional[str]:
...
def main():
    if len(sys.argv) not in [2, 3]:
        print("Usage: python3 generate_plugin_descriptor.py <plugin_name> [output_format]")
    ...
    if output_format not in ['yaml', 'json']:
        print("Error: Output format must be 'yaml' or 'json'")
        sys.exit(1)
    ...
    # Generate descriptor
    descriptor = extract_plugin_descriptor(plugin_path)
    if not descriptor:
